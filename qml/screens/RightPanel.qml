...
Rectangle {
    ...
    Flickable {
        ...
        Column {
            ...
            Column {
                ...
                VcStatRow {
                    showDivider: false
                    icon: "wifi"; label: qsTr("Connection")
                    value: root.conn.connected ? "TCP · MJPEG" : "—"
                }
                VcStatRow {
                    icon: "address"; label: qsTr("Address")
                    value: root.conn.connected
                           ? root.conn.host + " : " + root.conn.port : "—"
                }
                VcStatRow {
                    icon: "signal-graph"; label: qsTr("Signal")
                    ...
                    Row {
                        ...
                        Repeater {
                            ...
                            Rectangle {
                                ...
                                readonly property int q:
                                    root.conn.connected ? root.conn.linkQuality : -1
                                anchors.bottom: parent.bottom
                                width: 3
                                height: 13 * heights[index]
                                radius: 1
                                color: index > q ? Theme.fg4
                                    : q >= 2 ? Theme.statusLive
                                    : q === 1 ? Theme.statusWarn
                                    : Theme.statusError
                            }
                        }
                    }
                ...
                VcStatRow {
                    icon: "latency"; label: qsTr("Latency")
                    value: root.conn.connected && root.conn.frameIntervalMs > 0
                           ? root.conn.frameIntervalMs.toFixed(0) + " ms" : "—"
                    good: root.conn.connected && root.conn.frameIntervalMs > 0
                          && root.conn.frameIntervalMs < 34
                }
                VcStatRow {
                    icon: "bitrate"; label: qsTr("Bitrate")
                    // 0 here means "connected but no stream yet" — show "—", not 0.0
                    value: root.conn.connected && root.conn.bitrateMbps > 0
                           ? root.conn.bitrateMbps.toFixed(1) + " Mbps" : "—"
                }
                VcStatRow {
                    icon: "packet"; label: qsTr("Packet loss")
                    // TCP transport → no frame loss once frames are actually flowing
                    value: root.conn.connected && root.conn.frameWidth > 0 ? "0.0 %" : "—"
                    good: root.conn.connected && root.conn.frameWidth > 0
                }
                VcStatRow {
                    icon: "clock"; label: qsTr("Uptime")
            ...
            Column {
                ...
                VcStatRow {
                    showDivider: false
                    icon: "phone"; label: qsTr("Model")
                    value: root.conn.connected ? root.conn.deviceName : "—"
                }
                VcStatRow {
                    icon: "system"; label: qsTr("System")
                    value: (root.conn.connected && root.conn.deviceOs !== "")
                           ? root.conn.deviceOs : "—"
                }
                VcStatRow {
                    icon: "battery-full"; label: qsTr("Battery")
                    // never render the raw -1 sentinel → em dash when unknown
                    value: !(root.conn.connected && root.conn.battery >= 0)
                           ? "—"
                           : root.conn.charging
                             ? root.conn.battery + "% · " + qsTr("charging")
                             : root.conn.battery + "%"
                    good: root.conn.connected && root.conn.charging
                }
                VcStatRow {
                    icon: "camera"; label: qsTr("Lens")
                    value: root.conn.connected && root.conn.lens !== ""
                           ? root.conn.lens : "—"
                }
                VcStatRow {
                    icon: "capture"; label: qsTr("Capture")
                    value: root.conn.connected && root.conn.frameWidth > 0
                           ? root.conn.frameWidth + "×" + root.conn.frameHeight
                             + " · " + root.conn.fps + " fps"
                           : "—"
                }
            }
            ...
            Column {
                ...
                VcOptRow {
                    ...
                    showDivider: false
                    title: qsTr("Torch")
                    description: root.cc.torchAvailable ? qsTr("Rear flashlight")
                                                        : qsTr("No flash on front camera")
                ...
                VcOptRow {
                    title: qsTr("Focus lock"); description: qsTr("Freeze autofocus")
                    VcToggle {
                ...
                VcOptRow {
                    title: qsTr("Exposure lock"); description: qsTr("Lock AE / AWB")
                    VcToggle {
                ...
                VcOptRow {
                    ...
                    title: qsTr("HDR")
                    description: root.cc.hdrSupported ? qsTr("High dynamic range")
                ...
                RowLayout {
                    ...
                    VcButton {
                        ...
                        kind: "soft"; icon: "flip"; text: qsTr("Flip lens")
                    }
                    VcButton {
                        Layout.fillWidth: true
                        kind: "soft"; icon: "focus"; text: qsTr("Tap to focus")
                    }
                }
            }
            ...
            Column {
                ...
                VcOptRow {
                    showDivider: false
                    title: qsTr("Adaptive bitrate"); description: qsTr("Clamps to network")
                    VcToggle {
                ...
                VcOptRow {
                    title: qsTr("Hardware acceleration"); description: qsTr("GPU encode & decode")
                    VcToggle {
                ...
                VcOptRow {
                    title: qsTr("Adaptive resolution"); description: qsTr("Drop res before frames")
                    VcToggle {
                ...
                VcOptRow {
                    title: qsTr("Low-light boost"); description: qsTr("Brighten dim scenes")
                    VcToggle {
                ...
                VcOptRow {
                    title: qsTr("Noise reduction"); description: qsTr("Temporal denoise")
                    VcToggle {
                ...
                VcOptRow {
                    title: qsTr("Mirror image"); description: qsTr("Flip horizontally")
                    VcToggle {
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            TapHandler { onTapped: AppController.disconnectDevice() }
        }
    }
}
