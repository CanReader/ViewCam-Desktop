...
| `EventQueue` | In-memory ring spilled to JSON under `QStandardPaths::AppDataLocation`. Survives crash and offline; flushed on next launch. Hard-capped at 200 events, dropping oldest, so an offline machine cannot grow it unbounded. Never written at all under `VIEWCAM_ANALYTICS_DEBUG=1`, or a schema-verification run would leave events on disk for the next ordinary launch to send. |
